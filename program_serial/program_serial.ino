...
void setup() 
{
  Serial.begin(9600);   // Włączamy komunikacje przez Serial Monitor
  SPI.begin();      // Włączamy komunikacje przez SPI
  mfrc522.PCD_Init();   // Inicjujemy moduł RC522
  Serial.println("Zbliż tag RFID do czytnika");
  Serial.println();
...
void loop() 
{
  ...
  Serial.println();
  Serial.print("Wynik autoryzacji : ");
  content.toUpperCase(); //Przekształcenie wyniku na duże litery
  if (content.substring(1) == "BD 31 15 2B") // !!!TUTAJ WPISZ ID SWOJEGO TAGA!!!
  {
...
else if (content.substring(1) == "BD 31 15 2B") // !!!TUTAJ WPISZ ID DRUGIEGO TAGA!!!
  {
    Serial.println("Uzyskano dostęp");
    Serial.println();
    digitalWrite(8,HIGH); //Włączamy przekaźnik
